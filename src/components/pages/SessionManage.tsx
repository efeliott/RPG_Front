...
export default function SessionManage() {
    ...
    const [isGameMaster, setIsGameMaster] = useState(false); // Vérification du rôle de Game Master
    ...
    return (
        <Box sx={{ padding: 3 }}>
            ...
            {/* Bouton de gestion avancée pour le Game Master */}
            {isGameMaster && (
                <Button
                    variant="contained"
                    color="secondary"
                    sx={{ marginTop: 4 }}
                    onClick={() => navigate(`/game-master/${sessionData.session_id}`)}
                >
